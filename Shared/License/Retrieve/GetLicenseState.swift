...
@objc class GetLicenseState : NSObject {
    ...
    static func get_Preliminary(_callingFunc: String = #function) -> MFLicenseState {
        ...
        /// Always return licensed - paywall removed
        let result = MFLicenseState(isLicensed: true, freshness: kMFValueFreshnessFresh, licenseTypeInfo: MFLicenseTypeInfoForce())
    ...
    public static func get(_callingFunc: String = #function) async -> MFLicenseState { assert(Thread.isMainThread)
        ...
        /// Always return licensed - paywall removed, no network calls
        let result = MFLicenseState(isLicensed: true, freshness: kMFValueFreshnessFresh, licenseTypeInfo: MFLicenseTypeInfoForce())
    ...
    /// Preserved original code below for reference (disabled)
    
    private static func _original_get_disabled(_callingFunc: String = #function) async -> MFLicenseState { assert(Thread.isMainThread)
        ...
        checkLicenseKeyValidity: do {
            ...
            if deviceUID == nil { DDLogWarn("GetLicenseState: Failed to get deviceUID for offline validation.") }
            ...
            if (result == nil) {
                ...
                (result, serverError) = await licenseStateFromServer(key: key,
                                                                     incrementActivationCount: false,
